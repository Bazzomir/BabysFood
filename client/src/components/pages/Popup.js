// import React, { useState } from 'react';
// import { Modal, Card } from 'react-bootstrap';
// import jajca from '../assets/jajca.jpg';
// import { api } from '../../RESTApi/RestApi';
// import PropTypes from 'prop-types';

// export default function Popup(props) {

//     const recipe = props.recipe

//     // fetch(`${api.root}/recipes/myrecipes/${data.recipe._id}`)
//     //     .then(res => res.json())
//     //     .catch(err => alert(err));

//     function blablaFunkcija(props) {

//         return (
//             <div className='modal'>
//                 <Modal
//                     {...props}
//                     size="lg"
//                     aria-labelledby="contained-modal-title-vcenter"
//                     centered
//                 >
//                     <Modal.Header closeButton>
//                         <Modal.Title id="contained-modal-title-vcenter">
//                             <Card.Title>Card Title</Card.Title>
//                         </Modal.Title>
//                     </Modal.Header>
//                     <Modal.Body>
//                         <div className="container">
//                             <div className="row">
//                                 <div className="col-4">
//                                     <Card.Img variant="top" src={jajca} />
//                                     <Card.Title>{recipe.title}</Card.Title>
//                                     <span className="badge badge-success text-right" style={{ backgroundColor: 'green' }}>{recipe.category}</span>
//                                     <Card.Text>
//                                         {recipe.short_description}
//                                     </Card.Text>
//                                     <svg id="icons" xmlns="http://www.w3.org/2000/svg" fill="currentColor" className="bi bi-clock" viewBox="0 0 16 16">
//                                         <path d="M8 3.5a.5.5 0 0 0-1 0V9a.5.5 0 0 0 .252.434l3.5 2a.5.5 0 0 0 .496-.868L8 8.71V3.5z" />
//                                         <path d="M8 16A8 8 0 1 0 8 0a8 8 0 0 0 0 16zm7-8A7 7 0 1 1 1 8a7 7 0 0 1 14 0z" />
//                                     </svg>{recipe.preparation}
//                                     <svg id="icons" xmlns="http://www.w3.org/2000/svg" fill="currentColor" className="bi bi-person" viewBox="0 0 16 16">
//                                         <path d="M8 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6zm2-3a2 2 0 1 1-4 0 2 2 0 0 1 4 0zm4 8c0 1-1 1-1 1H3s-1 0-1-1 1-4 6-4 6 3 6 4zm-1-.004c-.001-.246-.154-.986-.832-1.664C11.516 10.68 10.289 10 8 10c-2.29 0-3.516.68-4.168 1.332-.678.678-.83 1.418-.832 1.664h10z" />
//                                     </svg>{recipe.people}
//                                     <svg id="icons" xmlns="http://www.w3.org/2000/svg" fill="currentColor" className="bi bi-star" viewBox="0 0 16 16">
//                                         <path d="M2.866 14.85c-.078.444.36.791.746.593l4.39-2.256 4.389 2.256c.386.198.824-.149.746-.592l-.83-4.73 3.522-3.356c.33-.314.16-.888-.282-.95l-4.898-.696L8.465.792a.513.513 0 0 0-.927 0L5.354 5.12l-4.898.696c-.441.062-.612.636-.283.95l3.523 3.356-.83 4.73zm4.905-2.767-3.686 1.894.694-3.957a.565.565 0 0 0-.163-.505L1.71 6.745l4.052-.576a.525.525 0 0 0 .393-.288L8 2.223l1.847 3.658a.525.525 0 0 0 .393.288l4.052.575-2.906 2.77a.565.565 0 0 0-.163.506l.694 3.957-3.686-1.894a.503.503 0 0 0-.461 0z" />
//                                     </svg>{recipe.views}
//                                 </div>
//                                 <div className="col-8">
//                                     <Card.Title>Card Title</Card.Title>
//                                     <Card.Text>
//                                         Some quick example text to build on the card title and make up the bulk of
//                                         the card's content.
//                                     </Card.Text>
//                                 </div>
//                             </div>
//                         </div>
//                     </Modal.Body>
//                 </Modal>
//             </div>
//         )
//     }
//     function ViewsUpdate() {
//         fetch(`${api.root}/recipes/myrecipes/view/${recipe._id}`)
//             .then(res => res.json())
//             .catch(err => alert(err))
//     }
//     const [popupShow, setPopupShow] = React.useState(false);

//     return (
//         <>
//             <button id="iconsButton" variant="success" onClick={() => { setPopupShow(true); ViewsUpdate() }}>
//                 <svg id="icons" xmlns="http://www.w3.org/2000/svg" fill="currentColor" className="bi bi-chevron-double-right" viewBox="0 0 16 16">
//                     <path fillRule="evenodd" d="M3.646 1.646a.5.5 0 0 1 .708 0l6 6a.5.5 0 0 1 0 .708l-6 6a.5.5 0 0 1-.708-.708L9.293 8 3.646 2.354a.5.5 0 0 1 0-.708z" />
//                     <path fillRule="evenodd" d="M7.646 1.646a.5.5 0 0 1 .708 0l6 6a.5.5 0 0 1 0 .708l-6 6a.5.5 0 0 1-.708-.708L13.293 8 7.646 2.354a.5.5 0 0 1 0-.708z" />
//                 </svg>
//             </button>

//             <blablaFunkcija show={popupShow} onHide={() => setPopupShow(false)} />
//         </>
//     )

// }

// Popup.propTypes = {
//     recipe: PropTypes.object.isRequired
// }
